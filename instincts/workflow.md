...
description: Global instincts for command pipelines, gates, plan-mode triggers, sprint mechanics, PO-decisions, push/commit policy and memory-override awareness. Loaded as Tier-1-global topic file alongside the slim index in `~/.claude/instincts.md`. Curated starter examples — adapt and extend through your own `/postmortem` runs.
type: instincts
scope: tier-1-global-topic
last_updated: 04.06.2026
---

# Command / Workflow / Sprint Instincts

Behavioural rules for sequencing commands, holding gate preconditions, planning multi-wave sprints, persisting product-owner decisions immediately, and respecting project memory overrides. Sorted by confidence (descending).
...
>
> **Multi-command sub-rule**: the pattern extends from sprint sessions to multi-command phase sessions. Trigger for session end: after 2 large commands (e.g. `/p3-ux` + `/p3-infra`) proactively commit + `/postmortem` + new session — even when the next command seems small.
>
> **Re-setup sub-rule**: A command iteration with extensive re-setup (e.g. wrong template → targets deleted and recreated with source-backup roundtrip + cleanup commits) counts as an additional command. Trigger for session end: after re-setup proactively split, even when only 1 nominal command ran.
>
> **Bulk-curation sub-rule**: The pattern also applies to sequential bulk-edit sessions without classical command structure (source curation, knowledge migration, bulk translation). Rule of thumb: one minor-version bump wave (≥4 curated files) corresponds to one command in terms of token consumption. After 2 version bumps: proactively split.

---

### G-044: Verify command-prescribed frontmatter against the project schema
**Confidence: 0.6** | Last confirmed: starter set

> **Rule**: When a command template instruction prescribes a concrete frontmatter value (e.g. `status: complete`), check it against the project schema (`PHASE_DOC_SCHEMA.md`, `MEMORY_SCHEMA.md`) before writing. On a mismatch, use the semantically closest project-allowed value.
>
> **Why**: A `/p4-sprint` command instructed `status: complete` for archived files; the project schema (`PHASE_DOC_SCHEMA.md`) only allows `{skeleton, draft, active, frozen, archived, living}`. This produced 1 lint-fail + 1 follow-up edit. `archived` (or `living` for conditionally-done docs with open conditions) was the correct semantic match.
>
> **How to apply**:
> 1. For frontmatter-writing command steps (especially archive/migrate operations) check the prescribed value against the `enum` list in the project schema.
> 2. Schema paths: `~/.claude/templates/PHASE_DOC_SCHEMA.md`, `~/.claude/templates/MEMORY_SCHEMA.md`.
> 3. If the command value isn't allowed: take the semantically closest allowed value — do not adopt it blindly.
>
> **Related**: G-032 in `agents.md` (verify optional paths) — same pre-apply verification philosophy. G-068 below (command-output language default) — analogous command-default vs project-override pattern.
>
> **Context**: All command steps that prescribe frontmatter values (status/kind/type fields). Command templates can lag behind the project schema state.
...
> 5. **Neither** → real stagnation: close the command, give a status update, or enter plan-mode for the next step.
>
...
> **Rule**: Before every `/gate-pX` command trigger, run `git status --porcelain` orchestrator-side. If the output is non-empty: assign and commit all sprint artefacts (tests, worker outputs, memory updates, gate reports, modified docs) explicitly before the gate command starts.
>
> **Why**: Gate commands (`/gate-p5`, `/gate-p6`, `/gate-p7`) have "working tree clean" as a hard precondition. Uncommitted changes at gate start block the command — you then have to do mid-command cleanup, which interrupts the command output messily.
>
> **How to apply**: A cleanup sequence right before each gate-command call:
> 1. Run `git status --porcelain`.
> 2. If empty: trigger the command.
> 3. If non-empty:
...
>
> **Related**: G-039 below (command-precondition sequence check) — same traceable-IDs philosophy. G-065 below (PoC-verdict cascade) — analogous cross-reference discipline.
>
...
### G-068: Command output language defaults to chat language instead of the project doc convention
**Confidence: 0.5** | Last confirmed: starter set

> **Rule**: Commands that produce user-facing docs (`/p0-*`, `/p1-*`, `/constitution`, `/gate-*`, `/p3-*`, …) tend to write prose in the **chat language** instead of the project's required **doc language**. Before triggering any command that writes a user-facing document, set the doc-language convention explicitly — via a CONSTITUTION pre-read, a project-memory check, or an explicit language clause in the subagent brief.
>
> **Why**: The chat context (e.g. German) dominates the output language model without an explicit counter-instruction, even when the project-wide standard is English. Constitution defaults and project memory are not auto-prefetched by the command workflow. Cost per lapse: 1 translate-commit + a review round + user frustration; at 3 lapses/session ~30-45 min wasted + repo noise.
>
> **How to apply**:
> 1. **Pre-command check (user-facing docs)**: before `/p0-*`, `/p1-*`, `/constitution`, `/gate-*`, `/p3-*`, check whether `docs/CONSTITUTION.md` exists and carries a language rule in Default/Inviolable. Prepend the resulting language convention to the command.
...
>
> **Anti-pattern**: trigger command → produce output in chat language → await user correction → push a translate-commit afterwards. 3× per session → a pattern, not a one-off.
>
> **Related**: G-044 above (command-prescribed frontmatter vs project schema) — analogous command-default vs project-override pattern.
>
> **Context**: Multi-lingual projects with a non-English-speaking PO + English-conventioned doc standards (DACH B2B SaaS, international reviewers/investors/contributors). Especially critical in P0–P3 where commands generate a lot of prose. Legal/DSGVO docs keep a domain-term whitelist (DSGVO, AVV, DSFA, VVT, TOM, Art. X DSGVO, § X TMG, BfDI, DSK, BSI).
...
> 3. `ExitPlanMode` after user approval → only then start the sub-command calls.
> 4. If criteria are NOT met: go straight to the per-story sub-command sequence.
>
> **Related**: G-035 above (clarify open points before ExitPlanMode); G-039 below (read command preconditions before plan-mode) — complementary.

---

### G-039: Read command preconditions before a plan-mode multi-command sequence
**Confidence: 0.4** | Last confirmed: starter set

> **Rule**: For plan-mode covering a multi-command sequence (3+ commands in a row, e.g. a wave-5 closeout with `/p5-acceptance` + `/p5-review` + `/p5-polish` + `/p5-docs` + `/gate-p5`), read the individual command definitions under `~/.claude/commands/{command}.md` **before** writing the plan. Especially: hard preconditions ("Gate-p5 passed", "working tree clean", "CI green") + the command's own ordering hint ("runs **between** X and Y").
>
> **Why**: A plan placed `/p5-polish` between `/p5-review` and `/p5-docs`. But `/p5-polish` has the hard precondition "Gate-p5 passed" — it **must** run after `/gate-p5`. The plan had to be corrected mid-command ("command convention beats plan sequence"). Reading the command defs up front would have prevented it — a command-correction round-trip is more expensive than 5× a command-spec read.
>
> **How to apply**:
> 1. In plan-mode phase 2 (design): for each planned sub-command read `~/.claude/commands/{command}.md`. Check "Preconditions" + the header text ("Runs between X and Y") + "Notes".
> 2. Adjust the plan sequence to the command constraints before `ExitPlanMode`.
> 3. For commands without plan-mode (direct in the command loop): not relevant.
>
> **Anti-pattern**: "deriving the order from sprint-N experience" — commands evolve; an order that was OK in sprint 1 can be wrong in sprint 3 because a new hard precondition was introduced.
>
...
> **Rule**: In autonomous pipelines with 3+ consecutive command commits, actively offer the user a push point instead of surfacing the backlog only at the pipeline end.
>
> **Why**: A pipeline session ended with 5 unpushed commits. Under a "user pushes themselves" policy, every commit is a user decision; pushing 5+ at once raises the diff-review load and removes the chance to fix interim states on the remote between commands.
>
> **How to apply**:
> 1. After every 2nd–3rd command commit in an autonomous pipeline: a compact push hint in the status report (e.g. "X commits unpushed, `git push origin main`").
> 2. At command transitions (e.g. after a sub-command, before a gate) mention the push option explicitly.
> 3. At the latest after 3 commits without a push hint: actively offer.
>
> **Context**: A "user pushes themselves" policy active AND auto-mode AND >2 consecutive commits without a push. Especially in multi-command sessions (4+ commands in a row).
...
> 3. **Execute the cross-file updates explicitly** and mention them in the POC.md output (section "Cascade updates"). Not "will be done in P3" — the next command wave is `/gate-p2`, which reads the cross-files.
...
### G-069: Ignore the TaskCreate reminder in a linear command flow
**Confidence: 0.4** | Last confirmed: starter set

> **Rule**: Ignore system `TaskCreate` reminders during linear command sequences (Read → Edit → Commit → index-update without concurrency or parallel streams). Tasks only pay off for multi-stream workflows (3+ agents in parallel, or substantial multi-tranche apply sequences with ≥4 phases that have verification steps between them).
>
> **Why**: The system reminders are prophylactic, not problem-driven — they fire every few minutes of activity regardless of the workflow. For a linear command script the path is clear (read context → delegate → apply → update indices → commit) and TaskCreate adds only setup overhead with no tracking benefit. Per reminder ignored: ~0 cost. Per reminder followed in a linear flow: ~30-60s of TaskCreate setup + TaskUpdate micromanagement = token burn with no benefit.
>
> **How to apply**:
> 1. On a "task tools haven't been used" reminder, check: is the current workflow linear (one command, clear sequence) or multi-stream (parallel agents, several apply tranches with verification, long research sequences)?
