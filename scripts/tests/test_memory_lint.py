...
class MemoryLintTest(unittest.TestCase):
    ...
    def test_the_shipped_default_severity_is_warn(self):
        ...
        makes the tracked promotion to `err` a deliberate red test — one failure,
        here, with the reason written on it — instead of a behaviour change that
        only shows up as an exit code somewhere in CI.
        """
