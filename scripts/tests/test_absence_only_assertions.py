    ...
    def test_classification_counts(self):
        ...
        growing paragraph:
          ...
          1371 / 0             08.09.2026 (CCP-1145, check-all.sh concurrency
                               ...
                               ConcurrentLockRedProofTest,
                               GitDirKeyedLockTest,
                               CaseAliasedPathsShareTheSameGitDirLockTest --
                               one method each; the last two added in a
                               code-review follow-up round to close a gap
                               where every other new test here ran only
                               against a non-git fixture). Three of the six
                               (StaleLockFromACrashedRunDoesNotBlockTest,
                               ConcurrentLockRedProofTest,
                               GitDirKeyedLockTest) also call the new
                               `self.popen_check_all(...)` helper, whose
                               name does NOT match RUN_HELPER_RE (it starts
                               "popen", not "run") -- the same naming-shape
                               blind spot recorded below for `self.push(...)`
                               -- but every one of the 6 ALSO calls
                               `self.run_check_all(...)` at least once, which
                               DOES match, so all 6 are in scope regardless.
                               None flagged: each asserts a positive
                               `returncode` equality (`assertEqual`), a
                               positive `assertTrue` (the two git-fixture
                               tests' own lock-directory/case-alias-resolves
                               checks), and/or a positive `assertIn` on
                               stdout/stderr; the `assertNotIn`/
                               `assertNotEqual` calls some of them also carry
                               are never the sole assertion.
          ...
          1176 / 0             31.08.2026: install.sh --verify stopped
                               ...
                               source checkout's own ignore rules
                               (`git check-ignore`), so most of its tests
                               drive `self.verify()` and stay out of scope;
                               +6 come from
                               TheExemptionIsDerivedFromTheSourceCheckoutTest,
                               whose six methods each install first via
                               `self.run_install`. None flagged; every one
                               asserts on a returncode plus a named path in
                               a named report block. Set proof rather than a
                               bare delta: the scanner was run against a
                               `git archive` of 8b824bf in a scratch tree and
                               against this one, keyed on (module,
                               Class.method) so line shifts cannot read as
                               moves -- exactly those six additions, zero
                               removals. The file count did not move: no new
                               module, the tests went into the existing
                               test_install_provenance.py.
...
class ScannedFilesCoverTheShippedScopeTest(unittest.TestCase):
    def test_scanned_files_cover_the_shipped_scope(self):
        ...
        Bumped 76 -> 77, 16.09.2026 (CCP-1214): added
        test_docs_boundary_default_deny.py, the docs/ default-deny guard.
        Proven an addition, not a swap: `git status --porcelain scripts/tests`
        showed one `??` line for the new file plus a ` M` on
        test_docs_dotfile_gitignore_coverage.py (whose repo-side gap-1 class
        was folded into the new module); nothing deleted, nothing renamed."""
        files = sorted(TESTS_DIR.glob("*.py")) + sorted((TESTS_DIR / "workitems").glob("*.py"))
        ...
        assert_set_matches(  # pin: set tests-corpus-files
            ...
            names,
            "the scripts/tests corpus (76 -> 77, 16.09.2026, CCP-1214: "
            "test_docs_boundary_default_deny.py added; proven an addition "
            "rather than a swap by `git status --porcelain scripts/tests` -- "
            "one `??` line (the new file) plus a ` M` on "
            "test_docs_dotfile_gitignore_coverage.py; nothing deleted, "
            "nothing renamed. Earlier trajectory: 75 -> 76, 11.09.2026, "
            "CCP-1178, test_agent_handover_write_boundary.py; "
