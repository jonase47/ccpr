...
Make sure `docs/quality/A11Y.md` exists as a **sub-index** with the standard header (Status / Last Updated / Key Decisions / Open Risks / Detail Files / Gate Notes — see `~/.claude/docs/PROJECT_PHASES.md`). If missing, create it with empty placeholders. The three `p6-a11y-*` sub-commands below each refresh their own row in this sub-index's **Detail Files** table.
...
After all `p6-a11y-*` sub-commands have run, summarise the A11Y sub-index into the phase index `docs/quality/QA.md`:
...
## Notes
- All three sub-commands can run in parallel (no dependencies)
