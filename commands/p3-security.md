...
Creates the complete security architecture: threat model, auth concept, data security, API security and developer checklist. Each area is its own sub-command.
...
The orchestrator owns the final `docs/HANDOVER.md` update for this P3 section. Sub-commands (`/p3-sec-*`) also contain a Handover Epilogue — this is intentional, so the sub-commands remain usable on their own. **However, when this orchestrator runs, the orchestrator's HANDOVER update is the authoritative one and supersedes individual sub-command updates.** Consolidate all `SECURITY.md` sub-sections into a single HANDOVER entry.
