...
>   casefold — so a re-run never duplicates an already-added item, and run `workitems similar
>   "<item>"` too, to catch a near-duplicate an exact title match misses and to fill in
>   `--checked-against`; record the assigned `**Work-Item:** WI-NNNN` id in the generated view, §4).
