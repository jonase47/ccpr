...
Develops the complete UX concept: navigation, wireframes, dark mode and accessibility. Each area is its own sub-command.
...
Make sure `docs/architecture/UX_CONCEPT.md` exists as a **sub-index** with the standard header (Status / Last Updated / Key Decisions / Open Risks / Detail Files / Gate Notes — see `~/.claude/docs/PROJECT_PHASES.md`). If missing, create it with empty placeholders. The four `p3-ux-*` sub-commands below each refresh their own row in this sub-index's **Detail Files** table.
...
The orchestrator owns the final `docs/HANDOVER.md` update for this P3 section. Sub-commands (`/p3-ux-*`) also contain a Handover Epilogue — this is intentional, so the sub-commands remain usable on their own. **However, when this orchestrator runs, the orchestrator's HANDOVER update is the authoritative one and supersedes individual sub-command updates.** Consolidate the four UX detail files into a single HANDOVER entry.
