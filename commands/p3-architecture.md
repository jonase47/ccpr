...
Designs the system architecture, selects the tech stack and documents architectural decisions. Each concern is its own sub-command.
...
If not provided: Sub-commands make their own recommendations.

## Flow

### 0. Ensure Phase Index Exists
Make sure `docs/architecture/ARCHITECTURE.md` exists with the standard phase-index header (Status / Last Updated / Key Decisions / Open Risks / Detail Files / Gate Notes — see `~/.claude/docs/PROJECT_PHASES.md`). If missing, create it with empty placeholders. The sub-commands below each refresh their own row in the **Detail Files** table.
...
The orchestrator owns the final `docs/HANDOVER.md` update for this P3 section. Sub-commands (`/p3-arch-*`) also contain a Handover Epilogue — this is intentional, so the sub-commands remain usable on their own. **However, when this orchestrator runs, the orchestrator's HANDOVER update is the authoritative one and supersedes individual sub-command updates.** Use the Wingman summary from §5 as the basis for the entry.
