...
- Structured store: `workitems create --title "<follow-up task>" --type fix --description
  ...
  never duplicates an already-created task, and run `workitems similar "<follow-up task>"` too, to
  catch a near-duplicate an exact title match misses and to fill in `--checked-against`; record the
  assigned `**Work-Item:** WI-NNNN` id in this file's `## Open Action Items`).
