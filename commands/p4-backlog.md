...
- A prose `docs/planning/BACKLOG.md` with actual story content exists **AND** `docs/workitems/`
  ...
  register — the work-state drift `docs/adr/ADR-0002-workitem-backend-contract.md` warns against).
  Instead, recommend running
...
`docs/adr/ADR-0002-workitem-backend-contract.md` for the adoption-guard rationale used by commands
that transition an *existing* store). `BACKLOG.md` becomes a generated **view** over the created
items, not their source of truth.
