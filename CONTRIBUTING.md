...
- **`-t .` is not optional**, and the failure mode is worth knowing because it is
  ...
  30.08.2026, 684 on 05.09., and 717 since later that day — unchanged by either
  06.09. cut, whose 32 resp. 6 new tests import cleanly with or without the flag, so
  the same addition lands on both sides of the subtraction and cancels. Each jump bought something — most came from sharing one
  ...
  **Re-measure these numbers when you change them, rather than adjusting one.** The
  ...
  found stale together six times — the file claimed 1691 / 1185 / 14 / ~510
