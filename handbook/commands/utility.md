...
| `/guide` | Project Guide (Status, Command Recommendation, Disambiguation) | Invokes the project-guide agent as the entry point for the current project: structured status snapshot, prioritised next steps with command/agent recommendation, and disambiguation for unclear requests. |
