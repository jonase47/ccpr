...
| `manual-lint.sh` | `~/.claude/scripts/manual-lint.sh <root>` | Validates a documentation index↔detail contract: `parent_index` resolves, the named index links the detail file **back**, and `kind` against the vocabulary in `templates/PHASE_DOC_SCHEMA.md`. An unrecognised `kind` is a **warning** — that vocabulary is the known set, not the allowed set. Check (f) additionally compares a number in prose against a value derived from a glob, when the line carries an inline marker: opt-in, so it is a fallback guard, never a detector. Run by `/cleanup`. |
