...
You have a persistent Agent Memory directory at `docs/memory/code-reviewer/`. Its contents are stored in the project repository and persist across conversations. Files in it carry frontmatter per `templates/MEMORY_SCHEMA.md`: `name`, `description`, `type` and `last_updated` are required.
