...
6. If active phase = P4/P5: `docs/planning/SPRINT.md` + most recent `docs/planning/sprint/SPRINT-XX.md`.
   ...
   treat those counts as the source of truth for phase status (e.g. "6/10 Done, 2 In Progress, 2
   Ready"), not SPRINT.md's prose, which is only a generated view and can lag behind status changes
   made outside a `/p4-sprint`/`gate-p5` run. If no such counts were passed in (prose-only project,
   or the invocation didn't provide them), fall back to SPRINT.md as before.
...
7. Phase-specific: `docs/planning/SPRINT.md` / `docs/planning/sprint/SPRINT-XX.md` (P4/P5) — or the
   orchestrator-provided work-item counts (ADR-0002 §8) if the project has adopted the structured
   store; those counts take precedence over SPRINT.md prose for phase status (see Status Snapshot).
