...
- **`.github/workflows/ci.yml`'s `check-all-macos` job now forwards a deny-list into CI via a
  ...
  04.09.2026) was the local pre-push hook (CCP-1137/17b), installed only where a contributor's
  machine has one and entirely absent on any machine that has not run it yet. The new
  step-level `env: CCPR_GATE_DENY_NAMES: ${{ secrets.CCPR_GATE_DENY_NAMES }}` on the "Run
  check-all.sh" step reads the same environment variable `scripts/artifact-gate.sh`'s own
  header already documents (lines 13–15) as the non-personal, non-distributed alternative to
  `~/.claude/memory-sync.json`'s `gate.denyNames`. GitHub does not expose repository secrets to
  a `pull_request` run triggered from a fork — the expression resolves to an **empty string**
  there, not an error — which `scripts/lib/discipline_gate.sh`'s own `[ -n
  "${CCPR_GATE_DENY_NAMES:-}" ]` check already treats the same as unset, falling through to the
  ordinary "not configured" state the E4 change above now names visibly rather than passing
  silently. **Measured before implementing** (see this item's work-item comment, 08.09.2026):
  GitHub Push Protection with custom patterns — the server-side alternative that would have
  replaced this AND the local pre-push hook — is not available on this repository
  (`GET /repos/{owner}/{repo}/secret-scanning/custom-patterns` → HTTP 404, and every
  `security_and_analysis` switch reports disabled), so this remains an audit layer that runs
  AFTER a push, not a preventive one. `scripts/tests/test_ci_workflow.py` gains
  `check_denylist_env`, wired into `lint_ci_workflow`, pinning the step's `env:` mapping as a
  SET of keys, not a count — the identical fix CCP-1149 already applied to
  `on.push.branches` (`check_push_branches`), because a length-only check cannot tell a
  removed key from a renamed one (same count, wrong key — the more dangerous regression, since
  it still LOOKS configured). `DenylistEnvMutationTest` is the mutation-based red proof: removal
  of the whole `env:` mapping, and a same-count key rename, are both shown to be caught.
  **The GitHub Actions secret itself does not exist yet and must be created by the repository
  owner**, under the reference name `ci.yml` already carries (`CCPR_GATE_DENY_NAMES` — a
  configuration KEY, not a tenant name, and GitHub Actions requires the reference name literally
  in the workflow YAML the same way any other `${{ secrets.X }}` usage does). What must NOT go
  into this repository, not even as an example, is the secret's CONTENT — the actual
  tenant/project names — which is exactly what the deny-list mechanism exists to keep out; that
  content is named in this item's own report instead, never here.
...
- **`docs/CONSTITUTION.md` v1.3 — the file's own deferred `skill` → `command` exception is closed,
  ...
  bump; two PO decisions today (08.09.2026) close it. Ten of the file's twelve prose occurrences
  of "skill" are renamed to "command", classified individually rather than swept — among them the
  Aspirational goal `Skill-footprint consolidation` → `Command-footprint consolidation`, whose
  stale "114-skill surface" figure is also corrected to the measured 116. The remaining two sit
  inside the **frozen v1.0 Changelog entry** below ("no breaking skill changes", "skill
  consolidation") and were reverted after the bump's first pass renamed them by mistake: that
  entry records what was ratified on 15.05.2026, and the same "a protocol is not rewritten
  retroactively" convention this file's own `CHANGELOG.md` entries already apply to themselves
  (two paragraphs down) applies to it too — a briefing error, corrected the same day (PO decision
  08.09.2026). Inviolable #5 — cited by name from six places (this file plus five ADR sites) — is
  renamed `skill-interface` → `command-interface` everywhere it is cited. `CHANGELOG.md`'s own
  three `skill-interface` occurrences and the archived `WI-0006.md` stay untouched: they record
  what was true on their date, and a protocol is not rewritten retroactively. **No Inviolable
  changed in substance — only Inviolable #5's name.** The frontmatter `related:` entry and the
  Lean-Track-sunset measurement both still pointed at `Manual/LEAN_TRACK.md`, a path CCP-1150
  retired; both now read `handbook/LEAN_TRACK.md`, the "2 deferred" that entry named. `scripts/
  tests/test_doc_counts_agree.py`'s `CommandCountAgreementTest` gains `docs/CONSTITUTION.md` as an
  ...
  **`docs/adr/ADR-0001-versioning-and-distribution.md` follows in full**, a same-day PO decision:
  it carried six more generic "skill" prose occurrences, none of them the already-renamed
  `skill-interface` identifier, all meaning a CCPR command — a half-renamed file (a renamed goal
  title two lines above "removing skills is a breaking change") was judged worse than deferring it
  whole, so all six are renamed. `ADR-0002` and `ADR-0004` carry no "skill" prose at all;
  `ADR-0009` still carries four generic occurrences (one of them the frozen `subskill:` schema-field
  key, the other three plain prose at lines 150, 184 and 198) — reported, not renamed, a scope
  question for a later PO decision rather than this one's to answer.

  **The check-(g) finding below resolved itself, and not the way it looked like it would.** The
  v1.3 rename had concentrated every surviving "skill" token in this file onto the one line
  quoting the historical name "skill-interface", making `scripts/manual-lint.sh` check (g)'s
  `docs/CONSTITUTION.md` `pathContains` exemption look no longer load-bearing under a measurement
  that adds `docs/` as a scanned root (0 vs. 0, `scripts/tests/test_manual_lint_check_g.py`'s
  `RealCorpusRegressionTest.test_out_of_scope_entries_become_load_bearing_once_their_path_is_
  scanned`) — a state this entry's own test names as the signal to move it into
  `KNOWN_SPENT_CONTEXT_ENTRIES`. Re-measuring the same test against the corrected file (the v1.0
  entry's two reverted "skill" occurrences above are on a line the `skill-interface` `lineContains`
  entry does not cover) instead found it load-bearing again: removing the `pathContains` exemption
  now reproduces exactly two new errors, both at the reverted line. The entry stays exactly where
  it was — `KNOWN_OUT_OF_SCOPE_CONTEXT_ENTRIES` — and `scripts/tests/test_manual_lint_check_g.py`
  is unchanged; the fix was the revert above, not a reclassification, and the suspected redundancy
  was a two-commit-old artefact of the bump's own briefing error rather than a durable property of
  the corpus.
...
- **`.gitignore` enumerated the working state under `docs/` where `artifact-gate.sh` and
  ...
  `docs/workitems-idmap.yml`, `docs/.*`), while the other two read
  `scripts/lib/docs-framework-allowlist.txt` and deny anything absent from it. Measured on
  `0003f46` by the new guard: **24 top-level entries, 145 paths**, 21 of them genuine working
  state — the nine Full-Track phase folders (`discovery/`, `concept/`, `validation/`,
  `architecture/`, `planning/`, `quality/`, `launch/`, `operations/`, `reviews/`), the
  Lean-Track and session artefacts (`instincts.md`, `BASELINE.md`, `FRAME.md`,
  `CLAUDE-lean.md`, `LEARNINGS.md`, `PROMOTION_BRIEF.md`, `TRACK_DECISION.md`) and five
  further folders (`concepts/`, `epics/`, `roadmaps/`, `user-stories/`, `lean-archive/`). The
  other three are the sweep erring wide on purpose and are reported rather than filtered:
  `README.md` and `somewhere/` occur only as illustrations inside comments, and `chore` is a
  false positive off `feat/fix/refactor/docs/chore`. Over-inclusion can only produce a spurious
  failure somebody reads and resolves; under-inclusion is a path the guard silently stops
  seeing, which is the failure mode this ticket is about. CCP-1214 itself reported **16**,
  sweeping `commands/` and `scripts/` for the phase folders and sprint artefacts and counting
  `BACKLOG.md` / `SPRINT.md` / `PROJECT_PLAN.md` at the top level where the shipped commands
  spell them under `planning/`; the figure here is the same method widened to `hooks/`,
  `agents/` and `templates/` and to every `docs/` path those files name, which is what the
  guard actually asserts. The one that mattered: `commands/postmortem.md` tells a maintainer
  to store project-specific instincts in `docs/instincts.md`, so the command that *ends* a CCPR
  session wrote an untracked, un-ignored file into CCPR's own repository — one `git add -A`
  from the accident CCP-1017 records as having already happened once, when 17 CCPR work items
  were committed and pushed into what every user reads as their own state.
  The rule is now `docs/*` plus one `!docs/` re-inclusion per allowlist entry.
  **This is not a reversal of CCP-1057's reasoning but its full width**: that item deliberately
  chose a pattern over an enumeration ("a pattern nobody has to remember to extend is what
  actually keeps pace with a script adding a new report next quarter") and picked `docs/.*`,
  which matches dotfiles only, while every artefact above is a normal filename.
  `git ls-files docs/` returns the same 22 files and the same five top-level entries as before
  — captured and diffed, nothing silently untracked. A project generated by
  `scripts/project-init.sh` still receives an *enumerated* block, deliberately: a user's `docs/`
  is their own working directory and may legitimately track what CCPR's never does.
  The `!docs/` lines are a **third** copy of the allowlist, a cost CCP-1017's PO decision named
  in advance ("derive one from the other or pin their agreement with a test"); a `.gitignore`
  cannot include another file, so the new `scripts/tests/test_docs_boundary_default_deny.py`
  reads both files independently and fails whichever is edited alone — adding a framework
  document is now two lines, and forgetting either is a test failure rather than a silent miss.
  That module also pins the form and order the mechanism depends on, both **measured in a
  throwaway repo rather than assumed**: `docs/*` + `!docs/adr/` works where `docs/` +
  `!docs/adr/` erases the whole tree (git never descends into an excluded directory), and a
  `!` line placed before `docs/*` is overruled by it. Every ignore probe passes `--no-index`,
  because `git check-ignore` is index-aware and reports a *tracked* path as not-ignored whatever
  the patterns say — without the flag every assertion about an allowlisted entry would have
  passed vacuously. `test_docs_dotfile_gitignore_coverage.py` loses its repo-side gap-1 class,
  which default-deny made true by construction; the removal was **measured, not assumed** (three
  `.gitignore` mutations: as shipped → green, `docs/*` removed → red, `docs/*` swapped back for
  `docs/.*` → green), and re-aiming it at CCP-1156's "artifact inside an allowlisted directory"
  hole was *tried and rejected on evidence* — that module's `DOTFILE_RE` anchors on `docs/\.`
  and structurally cannot produce such a path, proven by a mutation that left the suite green.
  That hole is real and unchanged (a `!docs/adr/` line re-opens the directory, and
  `install_docs()` copies from the filesystem regardless), so it is now *characterised* in the
  new module rather than left to a comment — stated, not fixed; CCP-1156 stays open.
