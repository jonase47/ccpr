...
- **`install.sh --verify`'s IGNORED block is grouped by `.gitignore` rule instead of listing
  ...
  unread report on the correct case, the same failure mode CCP-1166 already fixed once for the
  UNEXPECTED block. `path_is_source_ignored()` now reads `git check-ignore -v` instead of `-q`
  (the two flags refuse to combine) and groups by the exact `<source>:<line>:<pattern>` rule
  string it names, sorted by count descending — the same call that decides the exemption also
  supplies the grouping key, so there is no second, typed register of the rule shapes to drift
  from the first. The block now also prints unconditionally, for the same reason USER-OWNED
  already does: a reader must see how much was set aside without first having to trigger it. A
  path excused by a *different* rule — `docs/HANDOVER.md`, untracked in this repository — still
  lands on its own line, distinguishable from the bytecode-cache group in the same run.
